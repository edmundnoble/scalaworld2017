  ...
  object DivideCPS {
    ...
    object RandomAccessFinal {
      ...
      val interpreter = new RandomAccess[StateT[Either[IndexOutOfBoundsException, ?], String, ?]] {
        def write(i: Int, l: Int, data: String) = StateT[Either[IndexOutOfBoundsException, ?], String, Unit](s => if (s.length < i + l) (new IndexOutOfBoundsException).left else s.substring(0, i) + d + s.substring(i + l))
        def read(index: Int, length: Int) = StateT[Either[IndexOutOfBoundsException, ?], String, String](s => if (s.length < i + l) (new IndexOutOfBoundsException).left else s.substring(i, i + l - 1))
      }
